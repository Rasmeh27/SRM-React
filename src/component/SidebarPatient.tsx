// src/component/SidebarPatient.tsx
import { Link, useLocation } from "react-router-dom";
import { Home, History, IdCard, LogOut } from "lucide-react";
...
const NavItem = ({ to, icon: Icon, label }: ItemProps) => {
  ...
  return (
    ...
    >
      {/* Indicador activo (barra sutil a la izquierda) */}
...
export default function SidebarPatient() {
  return (
    // Lateral compacto con gradiente teal, sombras suaves y anillas coherentes con el resto del SRM
    <aside
    ...
    >
      {/* Logo SRM Paciente (coloca /public/logo-srm.svg o /public/logo-srm.png) */}
      <Link
        to="/patient"
      ...
      <nav className="flex flex-col items-center gap-2">
        <NavItem to="/patient" icon={Home} label="Inicio" />
        <NavItem to="/patient/prescriptions" icon={History} label="Mis recetas" />
        <NavItem to="/patient/profile" icon={IdCard} label="Perfil" />
      </nav>
